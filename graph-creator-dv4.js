document.onload = (function(d3, undefined){
  ...
  GraphCreator.prototype.updateGraph = function(){
    ...
    var enter = paths.enter()
      ...
      .classed("link", true)
      .attr("d", function(d){
        // console.log("intit d path second : M", d.source.x , d.source.y, d.target.x, d.target.y );
        // if(d.source.y > d.target.y) {

        // }
        var mx = d.source.x;
        var my = d.target.y;
        if (Math.abs(d.target.x - d.source.x) > 100) {
    ...
    // update existing nodes
    thisGraph.circles = d3Circles.data(thisGraph.nodes, function(d){ return d.id;});
...
**************************************************************************/
  ...
  GraphCreator.prototype.svgMouseUp = function(){
    var thisGraph = this,
        ...
        console.log('svg mosue up');
    if (state.justScaleTransGraph) {
      // dragged not clicked
      state.justScaleTransGraph = false;
    } else if (state.graphMouseDown && d3.event.shiftKey){
      // clicked not dragged from svg
      console.log('begin to add node');
      ...
      // var d3txt = thisGraph.changeTextOfNode(d3.select("svg").select(".circles").selectAll("circle").filter(function(dval){
      //   return dval.id === d.id;
      // }), d),
      // txtNode = d3txt.node();
